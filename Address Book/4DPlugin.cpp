...
#if VERSIONMAC
#define THIS_BUNDLE_ID @"com.4D.4DPlugin.miyako.AddressBook"
#include <AddressBook/AddressBook.h>
#include <AddressBook/ABAddressBookC.h>
#include "listener_addr.h"
listener_addr *listener = nil;
#endif
...
void _AddressBookListenerLoopCall4D()
{
...
void AddressBookListenerLoop(){
	
	PA_YieldAbsolute();	
	int AddressBookListenerProcess = PA_GetCurrentProcessNumber();
	PA_FreezeProcess(AddressBookListenerProcess);
	...
	{ 
		...
		if (!done){		
			...
			PA_NewProcess((void * )_AddressBookListenerLoopCall4D, 512*1024, listener.notificationType);
			PA_FreezeProcess(AddressBookListenerProcess);
...
void appleScriptExecuteCommand(NSString *fileName)
{
		...
		if(scriptPath){
			...
			if(scriptUrl){
				...
				if(scriptObject){
					...
					if(scriptIsCompiled)
						[scriptObject executeAndReturnError:&errInfo];
...
{
	appleScriptExecuteCommand(@"terminate");
}
...
void AB_Set_person_image(sLONG_PTR *pResult, PackagePtr pParams)
{
	...
	if(person)
	{
		...
		while (err == eER_NoErr) 
		{
			type = PA_GetPictureData(picture, ++i, NULL);
			err = PA_GetLastError();
			...
			if(err == eER_NoErr)
			{
				...
				len = (unsigned int)(type.fLength * 4) + sizeof(uint8_t);
				...
				if((uti.find(".png") != std::string::npos) 
				   || (uti.find(".pdf") != std::string::npos)
				   || (uti.find(".jpg") != std::string::npos)				   
				   || (uti.find(".tiff") != std::string::npos))
				{
					PA_Handle h = PA_NewHandle(0);
					PA_GetPictureData(picture, i, h);
					image = [[NSData alloc]initWithBytes:PA_LockHandle(h) length:PA_GetHandleSize(h)];
					PA_UnlockHandle(h);
					PA_DisposeHandle(h);
					break;					
				}
		...
		if(image)
		{
			if([person setImageData:(NSData*)image])
				success = [[ABAddressBook sharedAddressBook]saveAndReturnError:&error];
	...
	if(!Param1.getUTF16Length()){
		...
		if(methodId){
			...
			if(!listener){	
				...
				int AddressBookListenerProcessNumber = PA_NewProcess((void *)AddressBookListenerLoop, -512*1024, @"$AB Notification Process");				
				
				listener = [[listener_addr alloc]
							initWithMethodName:methodName 
							methodId:[NSNumber numberWithInt:methodId]
							processNumber:[NSNumber numberWithInt:AddressBookListenerProcessNumber]];
